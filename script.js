...
const API_KEY = "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"; // Hugging Face API Key

// Some placeholder sample prompts
...
(() => {
    ...
    // set dark theme if user explicitly chose dark theme or no preferance but the system default is dark theme
    const isDarkTheme = savedTheme === "dark" || (!savedTheme && systemPrefersDark);
    document.body.classList.toggle("dark-theme", isDarkTheme);
    // sun icon if dark theme, moon icon if light theme
    themeToggle.querySelector("i").className = isDarkTheme ? "fa-solid fa-sun" : "fa-solid fa-moon";
})();

// Toggle between light and dark themes
const toggleTheme = () => {
    // isDarkTheme is True if the theme turns dark and false if the theme turns light (removes the dark-theme which was already there)
...
const getImageDimensions = (aspectRatio, baseSize = 512) => {
    // split the aspectRatio string and turn each number to num type
    // so "16/9" becomes 16 and 9 for instance 
...
const createImageCards = (selectedModel, imageCount, aspectRatio, promptText) => {
    // clear any previous images in the grid container, prepare for new content
...
const handleFormSubmit = (e) => {
    // js handles everything without refreshing (default action = refresh/reload page)
    e.preventDefault();
    // Get form values
